...
- [x] **M11: 面板折叠/贴底 + 功能级加固** (控制面板可折叠细条与贴底融合；瓦片坐标基准按源投影 wprd=WGS84/webst=GCJ02；COMPLETED 释放 WakeLock 与前台态；运行中改路线守卫；Root 执行器 stderr 合流与严格检测；通知节流与四态标题；CoordinateTransform 单测)
